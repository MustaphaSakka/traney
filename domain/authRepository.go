...
/auth/verify?token={token string}
	...
	&client_id={client id from the current route}
	&account_id={account id from current route if available}

Sample: /auth/verify?token=aaaa.bbbb.cccc&routeName=MakeTransaction&client_id=2000&account_id=95470
*/
func buildVerifyURL(token string, routeName string, vars map[string]string) string {
	u := url.URL{Host: "localhost:8888", Path: "/auth/verify", Scheme: "http"}
	q := u.Query()
	q.Add("token", token)
